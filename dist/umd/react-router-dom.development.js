...
(function (global, factory) {
  ...
  function useBlocker(shouldBlock) {
    ...
    let fn = React__namespace.useCallback(() => {
      if (typeof shouldBlock === "function") {
        ...
        if (result && typeof result === "object") {
          ...
          return {
            shouldBlock: typeof shouldBlock === "function" ? shouldBlock : () => !!shouldBlock,
            unstable_skipStateUpdateOnPopNavigation: !!result.unstable_skipStateUpdateOnPopNavigation
          };
        }
        ...
        return {
          shouldBlock: function (_shouldBlock) {
            function shouldBlock() {
              return _shouldBlock.apply(this, arguments);
            }

            shouldBlock.toString = function () {
              return _shouldBlock.toString();
            };
    ...
    let [blocker, setBlocker] = React__namespace.useState(router.createBlocker(blockerKey, fn));
    const updateBlocker = React__namespace.useCallback(() => {
  ...
  function usePrompt(message, opts) {
    let {
      beforeUnload
    } = opts || {};
    let promptCallback = React__namespace.useCallback(() => {
      ...
      let shouldBlock = () => !window.confirm(message);
    ...
    let prevState = React__namespace.useRef(blocker.state);
    React__namespace.useEffect(() => {
    ...
    }, [blocker]); // User is leaving the domain or refreshing the page. Here we have to rely on
